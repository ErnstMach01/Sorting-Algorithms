package ics4x.insertionsort.UnitTest;
...
import ics4x.insertionsort.InsertionSort;
...
public class InsertionSortTest{                                     //Test case class for the sorting algorithm
    ...
    public InsertionSortTest(int[] data, boolean direction) {
        number = data;
        order = direction;
    }
    ...
    public void testCompare() throws Exception {                    //Test case for compare method
        ...
        if(order == true) {
            for (int i = compareLocation; i >= 0; i--) {
                if (number[i] > number[compareLocation]) {
                    sortedLocation--;
                }
            }
        } else {
    ...
    public void testSwap() throws Exception {                       //Test case for swap method
        for (int k = 0; k < number.length; k++) {
            for (int l = k; l >= 0; l--) {
                int swapValue = number[k];
                int swapLocationValue = number[l];
                Sort.swap(number, k, l);
                if (number[l] != swapValue && number[l+1] != swapLocationValue) {
                    assertTrue(false);
                }
            }
        }
